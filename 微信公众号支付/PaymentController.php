...
class PaymentController extends HomeController {
	...
	public function init() {
		...
		parent::init();
        include $_SERVER['DOCUMENT_ROOT'] . "/vendor/payment/WxPayPubHelper/WxPayPubHelper.php";
	...
	public function payAction() {
        ...
        $order = M('orders')->where(array('id' => $id,'order_status'=>2))->getOne();
        ...
        $orderGoods = M('orderGoods')->where(array('order_id' => $id))->getOne();//->select(array('where' => array('order_id' => $id)));
        $goodsName = $orderGoods['product_name'];
	...
	public function weixinAction()
	{
        ...
        if($this->isMobile()){
            ...
            $wxModel = D('Js','goods','home');
            $open_id = $wxModel->getOpenId();
//            print_r($result);
            $prepay = $wxModel->unifiedOrder($result['order_sn'], $result['current_debt'], $userip, $open_id,'河北残疾人基金会捐助');
                $jsPayInfo = $wxModel->GetJsApiParameters($prepay['prepay_id']);
//                echo $jsPayInfo;
                include $this->display('weixin2.html');
                exit;
        }
